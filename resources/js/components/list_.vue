<template>
    <div class="container mt-4">
        <h5 class="mb-4 text-danger">Note: <br> Run these commands = "php artisan db:seed --class=PlayersTableSeeder"<br> "php artisan db:seed --class=TeamsTableSeeder" <br>
            Database name is : "laravel_vue" </h5>
        <h3 class="mb-4 h2">Un-Assigned Player List</h3>

        <div class="">
            <template v-if="players && players.length > 0">
                <vuedraggable v-model="players" :group="{ name: 'players', pull: 'clone', put: false }"
                    class="list-group" @move="onPlayerMove" @end="onDropPlayer">
                            ...
                            <div class="card-body">
                                ...
                                <div>
                                    ...
                                    <vuedraggable v-model="team.players"
                                        :group="{ name: 'players', pull: true, put: true }" class="list-group"
                                        item-key="id" @end="">
                                        <template #item="{ element }">
...
// const handleDrop = (team) => async ({ item }) => {
//     // console.log(itme.id);
//     if (!item || team.players.some(player => player.id === item.id)) return;
...
//         const index = players.value.findIndex(player => player.id === item.id);
//         if (index !== -1) {
//             players.value.splice(index, 1);
//         }

//         const responseAdd = await fetch(`http://127.0.0.1:8000/api/update-player/${item.id}`, {
//             method: 'PATCH',
//             headers: { 'Content-Type': 'application/json' },
//             body: JSON.stringify({ team_id: team.id, status: 1 })
//         });
